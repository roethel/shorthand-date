...
(function () {
    ...
    ShorthandDate.prototype.getISODate = function (dateString) {
         ...
         */
            ...
            if (newDate == null) {
                this.error("Error: Cannot create JS Date from string " + dateString);
                return undefined;
            }
            if ( Object.prototype.toString.call(newDate) === "[object Date]" ) {
                // it is a date
                if (isNaN(newDate.getTime())) {
                    this.error("Error: date is not valid. dateString " + dateString);
                    return undefined;
                }
                else {
                    // date is valid
                ...
                this.error("Error: Could not create JS Date object from date string "  +dateString);
                return undefined;
            }
        }
    };
    ...
    /*
     * log (message : String, verbosity : Integer ) : String
     *
     * Logging function. Set the output level of log messages.
     * Just a simple wrapper around console.log
     * Takes an optional verbosity level as input.
     *
     * Need to make sure console is defined or IE will show some funky
     * behavior.
     */
     ...
     *
     * Simple wrapper around console.error. Checks if console is defined
     *
     */
    ShorthandDate.prototype.error = function (message) {
